...
import { IClientUserProfile, IClientProfile, IUser } from '@/lib/shared';
import {
  Pencil,
  X,
  Save,
...
const ProfileCard = ({ profile }: { profile: IClientProfile }) => {
  ...
  const handleRemovePhoto = async () => {
    try {
      ...
      // For now, just update the cache to remove the logo
      // TODO: Implement proper deleteProfileLogo method in the service
...
export default function ClientProfilePage() {
  ...
  const clientUserId = user?.clientUserId ?? '';

  const { data: profile, isLoading: isProfileLoading } =
    useQuery<IClientUserProfile>({
      queryKey: ['client-user-profile', clientUserId],
      queryFn: () => clientUserProfileService.getUserProfile(clientUserId),
      enabled: !!user?.id,
    });

  // Client profile query for company information
  ...
  const { mutate: updateProfile, isPending: isUpdatingProfile } = useMutation({
    mutationFn: (data: any) =>
      clientUserProfileService.updateUserProfileBasic(clientUserId, data),
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({
        queryKey: ['client-user-profile', clientUserId],
      });
      // Sync app context and localStorage so name/email show correctly on dashboard and other pages
      if (user) {
        const updatedUser = {
          ...user,
  ...
  const { mutate: updatePassword, isPending: isUpdatingPassword } = useMutation(
    {
      mutationFn: (data: { currentPassword: string; newPassword: string }) =>
        clientUserProfileService.changeUserPassword(clientUserId, data),
      onSuccess: () => {
  ...
  if (isProfileLoading || isClientProfileLoading) {
    return (
      ...
      <div className="space-y-6 p-4">
                ...
                <Card className="dark:bg-primary/10 bg-white">
                  <CardContent className="p-4">
                    <div className="text-muted-foreground flex h-48 items-center justify-center">
                      <p className="text-sm">No company profile available</p>
                    </div>
                  </CardContent>
                </Card>
              )}
                        ...
                        <Tooltip>
                          ...
                          <TooltipContent>
                            <p className="max-w-xs text-sm">
                              ...
                              and will be displayed to candidates and team
                              members.
                            </p>
                          </TooltipContent>
                        </Tooltip>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <Tooltip>
                          ...
                          <TooltipContent>
                            <p className="max-w-xs text-sm">
                              Your current job title within the organization.
                              This helps candidates understand your role and
                              authority level.
                            </p>
                          </TooltipContent>
                        </Tooltip>
        ...
        <div className="bg-primary/10 dark:bg-primary/20 border-primary/10 dark:border-primary/20 flex items-start rounded-lg border p-4">
          ...
          <p className="text-primary dark:text-primary/90 text-sm">
            ...
            candidate interactions. Keep your information up to date for the
            best experience.
          </p>
        </div>
      </div>
