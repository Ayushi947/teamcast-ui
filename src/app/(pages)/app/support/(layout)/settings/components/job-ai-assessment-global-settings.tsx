...
interface JobAiAssessmentGlobalSettingsDialogProps {
  open: boolean;
  ...
  isLoading?: boolean;
  onSave?: (
    values: Omit<FormValues, 'requiredSections'> & {
      requiredSections?: string[];
    }
  ) => Promise<void>;
}

export function JobAiAssessmentGlobalSettingsDialog({
  open,
  onOpenChange,
  initialValues,
  isLoading: _isLoading = false,
  onSave,
}: JobAiAssessmentGlobalSettingsDialogProps) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      defaultAssessmentDuration:
        initialValues?.defaultAssessmentDuration || 3600,
      defaultPassingScore: initialValues?.defaultPassingScore || 0.7,
      requiredSections: initialValues?.requiredSections?.join(', ') || '',
      maximumAttempts: initialValues?.maximumAttempts || 3,
      cooldownPeriod: initialValues?.cooldownPeriod || 7,
      maxAssessmentDuration: initialValues?.maxAssessmentDuration || 7200,
      assessmentBuffer: initialValues?.assessmentBuffer || 300,
      useCustomPrompts: initialValues?.useCustomPrompts ?? false,
      aiDifficulty: initialValues?.aiDifficulty || 'MEDIUM',
      maxSections: initialValues?.maxSections || 5,
      maxQuestionsPerSection: initialValues?.maxQuestionsPerSection || 5,
      proctoringEnabled: initialValues?.proctoringEnabled ?? true,
      maxWarnings: initialValues?.maxWarnings || 3,
      tabSwitchLimit: initialValues?.tabSwitchLimit || 3,
      copyPasteAllowed: initialValues?.copyPasteAllowed ?? false,
      videoRecordingEnabled: initialValues?.videoRecordingEnabled ?? true,
      minimumVideoLength: initialValues?.minimumVideoLength || 300,
      aiVideoAnalysisEnabled: initialValues?.aiVideoAnalysisEnabled ?? true,
      autoPublishOnSuccess: initialValues?.autoPublishOnSuccess ?? false,
      autoNotifyOnComplete: initialValues?.autoNotifyOnComplete ?? true,
  ...
  const onSubmit = async (values: FormValues) => {
    if (onSave) {
      setIsSaving(true);
      try {
        // Convert requiredSections string to array
      ...
      <DialogContent className="max-h-[90vh] max-w-5xl overflow-hidden">
        <DialogHeader className="border-b pb-4">
          <DialogTitle className="flex items-center gap-2 text-xl">
            ...
            Job AI Assessment Global Settings
          </DialogTitle>
        </DialogHeader>
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>
                            Max Assessment Duration (seconds)
                          </FormLabel>
                          <FormControl>
                            <Input
                              type="number"
        ...
        <div className="-mx-4 flex items-center justify-end gap-3 rounded-b-lg border-t px-4 pt-4 pb-4">
          ...
          <Button
            ...
            disabled={isSaving}
            className="flex items-center gap-2 bg-indigo-600 transition-colors hover:bg-indigo-700"
          >
            {isSaving ? (
