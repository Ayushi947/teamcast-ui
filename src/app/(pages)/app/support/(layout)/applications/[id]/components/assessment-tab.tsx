...
interface AssessmentTabProps {
  ...
  supportJobApplicationAiAssessment?: ISupportJobApplicationAiAssessment;
...
export function AssessmentTab({
  ...
  // Fetch candidate data with all assessments
  const { data: candidateData } = useQuery({
    queryKey: ['supportCandidate', candidateId],
    queryFn: () =>
  ...
  // Type for job AI assessment with all required properties
  // Using a flexible type that can handle both ISupportJobApplicationAiAssessment and candidate assessments
  type JobAiAssessmentDisplay = {
    id: string;
    status: string;
    result: string;
    score: number;
  ...
  // Use the job AI assessment from props or from candidate data
  const jobAiAssessments = useMemo((): JobAiAssessmentDisplay[] => {
    if (supportJobApplicationAiAssessment) {
      // Get full assessment from candidate data if available to get all properties
      const candidateAssessment = candidateData?.jobAiAssessments?.find(
        (a) => a.id === supportJobApplicationAiAssessment.id
      );

      // Convert ISupportJobApplicationAiAssessment to the format expected by the component
      const assessment: JobAiAssessmentDisplay = {
        id: supportJobApplicationAiAssessment.id,
        status: supportJobApplicationAiAssessment.status,
        result: supportJobApplicationAiAssessment.result,
        score: supportJobApplicationAiAssessment.score,
        startedAt: supportJobApplicationAiAssessment.startedAt,
        completedAt: supportJobApplicationAiAssessment.completedAt,
        duration: supportJobApplicationAiAssessment.duration,
        overallFeedback: supportJobApplicationAiAssessment.overallFeedback,
        recommendation: supportJobApplicationAiAssessment.recommendation,
        selectedForNextRound:
          supportJobApplicationAiAssessment.selectedForNextRound,
        strengths: supportJobApplicationAiAssessment.strengths || [],
        areasForImprovement:
          supportJobApplicationAiAssessment.areasForImprovement || [],
        sections: supportJobApplicationAiAssessment.sections || [],
        videoAnalysis: candidateAssessment?.videoAnalysis,
        technicalSkills: candidateAssessment?.technicalSkills,
        softSkills: candidateAssessment?.softSkills,
        // industriesFit and jobRolesFit are not available in ISupportCandidate.jobAiAssessments type
        industriesFit: undefined,
        jobRolesFit: undefined,
    ...
    // Fallback to candidate's job AI assessments if available
    return (candidateData?.jobAiAssessments || []).map((assessment) => ({
      id: assessment.id,
      ...
      selectedForNextRound: false, // Not available in simplified type
      ...
      industriesFit: undefined, // Not available in simplified type
      jobRolesFit: undefined, // Not available in simplified type
      jobPosting: assessment.jobPosting,
    }));
  }, [supportJobApplicationAiAssessment, applicationData, candidateData]);

  useEffect(() => {
    if (!jobAiAssessments.length) {
      return;
    }
    ...
    setJobAiActiveTab((prev) => {
      ...
      jobAiAssessments.forEach((assessment) => {
        if (!assessment?.id) {
          return;
        }
  ...
  // Use onboarding assessment from props or from candidate data
  const latestOnboardingAssessment = useMemo(():
    ...
    if (candidateData) {
      const assessment = getLatestOnboardingAssessment(candidateData);
      // Ensure it's properly typed as ICandidateOnboardingAssessment
      return assessment as ICandidateOnboardingAssessment | undefined;
    }
    return undefined;
  }, [onboardingAssessment, candidateData]);

  const resumeAssessments = candidateData?.resumeAssessments || [];
  ...
  return (
    <div className="space-y-6">
        ...
        <Card className="bg-card shadow-none hover:shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <RecommendationBadge
                recommendation={resumeAssessments[0].recommendation}
              />
            </div>
            <CardDescription className="mt-1 text-sm text-gray-600 dark:text-gray-400">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {resumeAssessments[0].overallFeedback}
              </p>
            </CardDescription>
          </CardHeader>
                  ...
                  <div className="mb-6">
                    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                      <div>
                        ...
                        <StrengthsCard
                          values={resumeAssessments?.[0]?.strengths || []}
                        />
                      </div>
                      <div>
                        <h3 className="mb-3 text-base font-semibold text-gray-900 dark:text-white">
                        ...
                        <AreasOfImprovementsCard
                          values={
                            resumeAssessments?.[0]?.areasForImprovement || []
                          }
                        />
                      </div>
                    </div>
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div>
                              ...
                              <CommonTags
                                values={
                                  latestOnboardingAssessment?.technicalSkills ||
                                  []
                                }
                              />
                            </div>
                            <div>
                              ...
                              <CommonTags
                                values={
                                  latestOnboardingAssessment?.softSkills || []
                                }
                              />
                            </div>
                            <div>
                              ...
                              <CommonTags
                                values={
                                  latestOnboardingAssessment?.industriesFit ||
                                  []
                                }
                              />
                            </div>
                            <div>
                              ...
                              <CommonTags
                                values={
                                  latestOnboardingAssessment?.jobRolesFit || []
                                }
                              />
                            </div>
                          </div>
      ...
      {/* Job AI Assessments */}
      {jobAiAssessments && jobAiAssessments.length > 0 ? (
        jobAiAssessments
          .filter((assessment) => {
          ...
          .map((assessment, index) => {
            ...
            const activeJobAiTab = jobAiActiveTab[assessment.id] ?? 'overview';
        ...
        <div className="flex items-center gap-2 rounded-lg border border-orange-100 bg-orange-50 p-3 dark:border-orange-800 dark:bg-orange-900/20">
          <AlertTriangle className="h-4 w-4 flex-shrink-0 text-orange-500" />
          <span className="text-sm text-gray-900 dark:text-white">
            No Job AI assessments found
          </span>
        </div>
      )}
    </div>
