...
const ProfileCard = ({
  ...
  const calculateCompletionPercentage = () => {
    ...
    if (resume?.phone) completed++;
    if (resume?.location) completed++;
            ...
            <>
              ...
              {hasValidationErrors ? (
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div className="inline-block">
                      <Button
                        ...
                        className="cursor-not-allowed bg-gray-400 text-gray-600 hover:bg-gray-400 dark:bg-gray-600 dark:text-gray-400 dark:hover:bg-gray-600"
                      >
                  ...
                  <TooltipContent className="bg-gray-100 text-gray-900 dark:bg-gray-900 dark:text-white">
                    <p className="text-sm">
                      Please fix phone number validation before saving
                    </p>
                  </TooltipContent>
                </Tooltip>
              ) : (
                <Button
                  onClick={onSave}
                  disabled={isSaving}
                  className="bg-[#6E55CF] text-white hover:bg-[#6E55CF]/90 dark:bg-[#6E55CF] dark:text-white dark:hover:bg-[#6E55CF]/90"
                >
                  <Save className="mr-2 h-4 w-4" />
                  {isSaving ? 'Saving...' : 'Save Changes'}
                </Button>
...
export default function CandidateProfilePage() {
  ...
  // Data fetching
  const isPartnerResource = user?.role === UserRoleEnum.PARTNER_RESOURCE;
  const partnerUserId = user?.partnerUserId;

  // Partner resource profile
  const { data: partnerProfile, isLoading: isPartnerProfileLoading } = useQuery(
    {
      queryKey: ['partner-user-profile', partnerUserId],
      queryFn: () =>
        partnerUserProfileService.getUserProfile(partnerUserId || ''),
      enabled: isPartnerResource && !!partnerUserId,
    }
  );
  ...
  const { data: resume, isLoading: isResumeLoading } = useQuery<IResume>({
    ...
    enabled: !isPartnerResource && userType === UserTypeEnum.CANDIDATE,
  });
  ...
  useEffect(() => {
    if (isPartnerResource && partnerProfile) {
      setProfileData({
        name: partnerProfile.name || '',
        sex: '', // Not available for partner
        jobTitle: partnerProfile.jobTitle || '',
        email: partnerProfile.email || '',
      });
    } else if (!isPartnerResource && profile) {
      setProfileData({
        name: profile.name || '',
        sex: profile.sex || '',
  ...
  }, [isPartnerResource, partnerProfile, profile]);

  useEffect(() => {
    if (!isPartnerResource && resume) {
      setResumeFields({
        phone: resume.phone || '',
        location: resume.location || '',
      });
    }
  }, [isPartnerResource, resume]);

  // Mutations
  const {
    mutate: updateCandidateProfile,
    isPending: isUpdatingCandidateProfile,
  } = useMutation({
    mutationFn: (data: any) => candidateProfileService.updateBasicProfile(data),
    onSuccess: async (_data, variables) => {
      // Sync app context and localStorage so name shows correctly on other pages
  ...
  const { mutate: updateResume, isPending: isUpdatingResume } = useMutation({
    mutationFn: (data: any) => candidateResumeService.updateResume(data),
    onSuccess: async () => {
      // Invalidate and immediately refetch to update UI
      await Promise.all([
  ...
  const { mutate: updatePartnerProfile, isPending: isUpdatingPartnerProfile } =
    useMutation({
      mutationFn: (data: any) =>
        partnerUserProfileService.updateUserProfileBasic(
          partnerUserId || '',
          data
        ),
  ...
  const {
    mutate: changePartnerPassword,
    isPending: isChangingPartnerPassword,
  } = useMutation({
  ...
  // Add candidate password change mutation
  const {
    mutate: changeCandidatePassword,
    isPending: isChangingCandidatePassword,
  } = useMutation({
    mutationFn: (data: { currentPassword: string; newPassword: string }) =>
      candidateProfileService.changePassword(data),
    onSuccess: () => {
      toast.success('Password updated successfully');
      setCurrentPassword('');
      setNewPassword('');
  ...
  const handlePhoneValidation = (error: string | null) => {
  ...
  const handlePasswordUpdate = () => {
    ...
    if (isPartnerResource) {
      ...
      changeCandidatePassword({ currentPassword, newPassword });
    }
  };
  ...
  // Status checks
  const isSaving =
    isUpdatingCandidateProfile ||
    isUpdatingResume ||
    isUpdatingPartnerProfile ||
    isChangingPartnerPassword ||
    isChangingCandidatePassword;

  // Loading
  if (
    (isPartnerResource && isPartnerProfileLoading) ||
    (!isPartnerResource && (isProfileLoading || isResumeLoading))
  ) {
    return (
    ...
    <TooltipProvider>
      <div className="space-y-6 p-4">
        <ProfileHeader
          ...
          isSaving={isSaving}
          hasValidationErrors={!!phoneValidationError}
        />
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
            ...
            <div className="space-y-6 lg:col-span-4">
              ...
              {isProfileLoading ? (
                ...
                <ProfileCard profile={profile} resume={resume} />
              ) : (
                <Card className="dark:bg-primary/10 bg-white">
                  <CardContent className="p-4">
                    <div className="text-muted-foreground flex h-48 items-center justify-center">
                      <p className="text-sm">No candidate profile available</p>
                    </div>
                  </CardContent>
                </Card>
              )}
              ...
              <Card className="dark:bg-primary/10 bg-white">
                ...
                <CardContent className="p-6">
                  <div className="space-y-4">
                        ...
                        <Tooltip>
                          ...
                          <TooltipContent>
                            <p className="max-w-xs text-sm">
                              ...
                              and will be displayed to employers and team
                              members.
                            </p>
                          </TooltipContent>
                        </Tooltip>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <Tooltip>
                          ...
                          <TooltipContent>
                            <p className="max-w-xs text-sm">
                              Your current or desired job title. This helps
                              employers understand your career goals and
                              expertise.
                            </p>
                          </TooltipContent>
                        </Tooltip>
                    ...
                    {/* Only show for individual candidate */}
                    {!isPartnerResource && (
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          <Label
                          ...
                          <Tooltip>
                            ...
                            <TooltipContent>
                              <p className="max-w-xs text-sm">
                                Your gender identity for demographic purposes
                                and to help with inclusive hiring practices.
                              </p>
                            </TooltipContent>
                          </Tooltip>
                        </div>
              ...
              <Card className="dark:bg-primary/10 bg-white">
                ...
                <CardContent className="p-6">
                  <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <Tooltip>
                          ...
                          <TooltipContent>
                            <p className="max-w-xs text-sm">
                              Your primary email address for communications with
                    ...
                    {/* Only show phone for individual candidate */}
                    {!isPartnerResource && (
                      <div className="space-y-2">
                        <ContactPhoneInput
                          ...
                          onValidationError={handlePhoneValidation}
                          disabled={!isEditing}
        ...
        <div className="bg-primary/10 dark:bg-primary/20 border-primary/10 dark:border-primary/20 flex items-start rounded-lg border p-4">
          ...
          <p className="text-primary dark:text-primary/90 text-sm">
            Your profile information is used for job applications and employer
            interactions. Keep your information up to date for the best
            opportunities.
          </p>
        </div>
      </div>
