...
const AssessmentOverview = ({ sections }: { sections: Section[] }) => (
  ...
  >
    <p className="text-muted-foreground mb-4 text-sm">
      ...
      evaluate your skills and experience required for the job requirements.
    </p>
...
const JobAiInstructionsPageContent = () => {
  ...
  const playWelcomeAudio = async () => {
    const welcomeText = `${assessment?.jobAiAssessmentSettings?.greetingMessage}.
  ...
  useEffect(() => {
    ...
    switch (assessment.status) {
      case JobAiAssessmentStatusEnum.AI_INITIALIZATION_IN_PROGRESS:
        router.replace(
          `/app/candidate/assessments/ai/check?id=${assessmentId}`
        );
        break;
      case JobAiAssessmentStatusEnum.AI_INITIALIZATION_COMPLETED:
        playWelcomeAudio();
        break;
      case JobAiAssessmentStatusEnum.CANDIDATE_ASSESSMENT_IN_PROGRESS:
        router.replace(
          `/app/candidate/assessments/ai/${assessmentRoute}?id=${assessmentId}`
        );
        break;
      default:
  ...
  const handleStartAssessment = async () => {
    ...
    try {
      const assessmentRoute = getAssessmentRoute();
      const data =
        await candidateJobAiAssessmentService.startAssessment(assessmentId);
      if (
        data.status ===
...
const JobAiInstructionsPage = () => {
  return (
    <Suspense fallback={<LoadingSpinner />}>
      <JobAiInstructionsPageContent />
    </Suspense>
  );
};

export default JobAiInstructionsPage;
