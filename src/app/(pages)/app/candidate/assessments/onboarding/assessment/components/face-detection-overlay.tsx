...
export const FaceDetectionOverlay = ({
  ...
  // NEW: Track multiple person violations
  const consecutiveNoFaceFramesRef = useRef<number>(0);
  const multiplePersonReportedRef = useRef<boolean>(false);

  // NEW: Grace period - don't show warnings immediately after assessment starts
  ...
  const initializeFaceDetector = useCallback(async () => {
    ...
    try {
      ...
      // These are informational messages that clutter the console
      ...
      console.error = (...args) => {
        ...
        if (
          ...
          // Log suppressed messages at debug level using logger
          logger.debug('Suppressed MediaPipe/TensorFlow error:', message);
          return;
        }
        // For real errors, log with logger and also pass to original console
        if (args.length > 1) {
          logger.error('MediaPipe initialization error:', args);
        } else if (args.length === 1) {
          logger.error('MediaPipe initialization error:', args[0]);
        } else {
          logger.error('MediaPipe initialization error');
        }
      ...
      console.warn = (...args) => {
        ...
        if (
          ...
          // Log suppressed messages at debug level using logger
          logger.debug('Suppressed MediaPipe/TensorFlow warning:', message);
          return;
        }
        // For real warnings, log with logger and also pass to original console
        if (args.length > 1) {
          logger.warn('MediaPipe initialization warning:', args);
        } else if (args.length === 1) {
          logger.warn('MediaPipe initialization warning:', args[0]);
        } else {
          logger.warn('MediaPipe initialization warning');
        }
  ...
  const startFaceDetection = useCallback(() => {
    ...
    logger.info('Starting face detection monitoring with grace period', {
      assessmentId,
      isEnabled,
    ...
    detectionIntervalRef.current = setInterval(async () => {
      ...
      const elapsed = startTimeRef.current
        ...
        : Infinity;
      const inGracePeriod = elapsed < GRACE_PERIOD_MS;

      const hasFace = await detectFaceCombined();
      setFaceDetected(hasFace);

      // NEW: Check for multiple people using MediaPipe detector
      if (faceDetector && videoRef.current && !mediaPipeError) {
        try {
          ...
          if (
            ...
            // FIX: Use detectForVideo() for VIDEO mode, not detect()
            const detections = faceDetector.detectForVideo(video, timestamp);
              ...
              if (!multiplePersonReportedRef.current) {
                ...
                import('@/lib/services/services').then(
                  ({ candidateOnboardingAssessmentService }) => {
                    candidateOnboardingAssessmentService.proctor(
                      assessmentId,
      ...
      if (!hasFace) {
        ...
        // Only report violations and show warnings after grace period
        if (!inGracePeriod) {
          // REPORT NO FACE VIOLATION after 3 consecutive frames (3 seconds)
          if (consecutiveNoFaceFramesRef.current === 3) {
            logger.warn('No face detected for 3 seconds (after grace period)');
          ...
          // Show warning only after grace period
          ...
          // Still in grace period - don't show warning but log for debugging
          logger.debug('No face detected but still in grace period', {
            elapsed,
            remaining: GRACE_PERIOD_MS - elapsed,
          });
        }
      } else {
        consecutiveNoFaceFramesRef.current = 0; // Reset counter
