...
const InterviewPageContent = () => {
  ...
  const [currentQuestion, setCurrentQuestion] =
    useState<ICandidateOnboardingAssessmentQuestion | null>(null);
  ...
  const [isMicEnabled, setIsMicEnabled] = useState(true); // Track microphone enabled state
  ...
  const {
    ...
    isMicEnabled: isMicEnabled, // Pass mic state so hook stops sending audio when disabled
  ...
  }, [streamingTranscript, currentQuestion?.id]);
  ...
  useEffect(() => {
    const fetchAssessment = async () => {
      ...
      try {
        ...
        const proctoring =
          assessment.onboardingAssessmentSettings?.proctoringEnabled || false;
  ...
  useEffect(() => {
      ...
      if (document.hidden && !isHandlingViolation) {
        ...
        try {
          ...
          if (proctoringEnabled) {
            ...
            await candidateOnboardingAssessmentService.proctor(
              assessment?.id || '',
              'TAB_SWITCHED'
            );
    ...
    const handleWindowBlur = async () => {
      ...
      const WINDOW_BLUR_LIMIT =
        assessment?.onboardingAssessmentSettings?.tabSwitchLimit || 3;
  ...
  useEffect(() => {
    ...
    const handleBrowserBack = () => {
      // Cleanup media resources when browser back button is pressed
    ...
    const handleBeforeUnload = () => {
      // Cleanup media resources when page is about to unload
      if (videoRecordingEnabled) {
        cleanupMediaStreams();
      }
    };
  ...
  }, [isStarted, isInterviewComplete, videoRecordingEnabled]);
  ...
  useEffect(() => {
    ...
    // Update current question ID ref for video chunk tracking
  ...
  useEffect(() => {
    ...
    const hasStartedBefore = sessionStorage.getItem(
      `teamcast-onboarding-assessment-${assessmentId}-started`
    );
    ...
    if (hasStartedBefore === 'true' && !isStarted) {
      ...
      router.replace('/app/candidate/assessments/onboarding/check');
      return;
    }
  ...
  const uploadChunk = async (chunk: Blob) => {
    ...
    try {
      ...
      const response =
        await candidateOnboardingAssessmentService.getPresignedUrl(
          assessmentId,
          chunkIndex
        );
      const { presignedUrl, gcsUri } = response;

      if (!presignedUrl) {
        throw new Error('Failed to get presigned URL');
      }

      // Use GCS URI from backend response (includes correct chunk index in filename)
      // Fallback to extracting from URL for backward compatibility
      let finalGcsUri = gcsUri;
      if (!finalGcsUri) {
        // Fallback: Extract GCS URI from presigned URL (legacy behavior)
        const gcsUrl = new URL(presignedUrl);
        const pathParts = gcsUrl.pathname.split('/').filter(Boolean);
        const bucketName = pathParts[0];
        const filePath = pathParts.slice(1).join('/');
        finalGcsUri = `gs://${bucketName}/${filePath}`;
      }
      ...
      // Record chunk upload in backend and trigger analysis
      const recordResponse =
        await candidateOnboardingAssessmentService.recordChunkUpload(
          assessmentId,
          chunkIndex,
          finalGcsUri,
  ...
  useEffect(() => {
    ...
    const handleAutoMute = () => {
      try {
        const audioTracks = microphoneStreamRef.current?.getAudioTracks() || [];
        if (audioTracks.length === 0) return;

        const aiStartedSpeaking = isSpeaking && !previousIsSpeakingRef.current;
        const aiStoppedSpeaking = !isSpeaking && previousIsSpeakingRef.current;

        // AI started speaking - mute candidate to prevent background noise
        if (aiStartedSpeaking) {
          const allTracksEnabled = audioTracks.every((track) => track.enabled);
          if (allTracksEnabled) {
        ...
        // AI stopped speaking - unmute candidate so they can answer
        if (aiStoppedSpeaking && wasAutoMutedRef.current) {
          const allTracksDisabled = audioTracks.every(
            (track) => !track.enabled
          );
        ...
        // If user manually unmutes while AI is speaking, clear the auto-mute flag
        if (
          isSpeaking &&
          audioTracks.some((track) => track.enabled) &&
          wasAutoMutedRef.current
        ) {
          wasAutoMutedRef.current = false;
          setIsMicEnabled(true);
          logger.info(
            '🔄 User manually unmuted during AI speech - clearing auto-mute flag'
          );
        }

        // If user manually mutes while AI is not speaking, clear the auto-mute flag
        if (
          !isSpeaking &&
        ...
        // Sync state with actual track state (in case of external changes)
        const currentMicState = audioTracks.some((track) => track.enabled);
        if (currentMicState !== isMicEnabled) {
  ...
  // NEW: Start listening using Google Cloud Streaming STT
  const startListening = async () => {
    ...
    if (!sttConnected) {
      logger.error('❌ Streaming STT not connected');
      toast.error('Speech recognition not ready. Please wait...');
      return;
    }

    // CRITICAL: Do NOT start listening while AI is speaking
    if (isSpeaking) {
      logger.warn(
        '⏸️ Cannot start listening while AI is speaking - waiting...'
      );
      // Wait for AI to finish speaking, then try again
      await waitUntilSpeechFinished();
      logger.info('✅ AI finished speaking, now starting listener');
    }

    // Reset transcript for new question (already done in useEffect, but ensuring here)
    ...
    // Force stop any existing listener before starting new one
    if (isListeningRef.current || sttListening) {
      logger.warn(
        '⚠️ Speech recognition already active, force stopping before restart'
      );
      stopListening();
      // Wait longer for full cleanup (WebSocket close + backend cleanup)
    ...
    try {
      ...
      logger.error('❌ Error starting streaming STT:', error);
      toast.error('Failed to start speech recognition. Please try again.');
    }
  };

  // NEW: Stop listening using Google Cloud Streaming STT
  const stopListening = () => {
    try {
      ...
      if (isListeningRef.current || sttListening) {
        ...
        logger.info('Speech recognition was not running');
      }
    } catch (error) {
  ...
  const handleSubmitAnswer = async (answer: string) => {
    try {
      ...
      const response = await candidateOnboardingAssessmentService.submitAnswer(
        assessmentId,
        currentQuestion.id,
        answer
      );
      ...
      // Update the question state AND the ref immediately
      setCurrentQuestion(response.nextQuestion);
      currentQuestionIdRef.current = response.nextQuestion?.id;
      logger.info('Updated current question state and ref', {
  ...
  const handleInterviewComplete = async () => {
    ...
    try {
      ...
      if (assessment?.id) {
        await candidateOnboardingAssessmentService.submitAssessment(
          assessment.id
        );
      ...
      await activityLogService.createActivityLog({
        ...
        description: `${candidateName}'s Onboarding Assesment Completed Successfully`,
        metadata: {
  ...
  const sendHeartbeat = async (assessmentId: string, timeRemaining: number) => {
    ...
    try {
      ...
      await candidateOnboardingAssessmentService.heartbeat(
        assessmentId,
        duration,
        'IN_PROGRESS'
      );
