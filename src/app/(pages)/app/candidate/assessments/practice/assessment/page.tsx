...
const InterviewPageContent = () => {
  ...
  const { user } = useApp();
  const isLoggedIn = !!user && user.type === UserTypeEnum.CANDIDATE;

  // Set the system is initialized to false
  const [systemInitialized, setSystemInitialized] = useState(false);
  const [assessment, setAssessment] = useState<IPublicPracticeAssessment | any>(
    null
  );
  ...
  const {
    ...
    isMicEnabled: true, // Always enabled
    requireAuth: isLoggedIn, // Only require auth if user is logged in
    suppressMicWarnings: true, // Suppress mic warnings since practice assessments don't use mic toggle
    onTranscript: (result) => {
  ...
  // Note: Analytics service may not be needed for practice assessments
  ...
  // Initialize audio-only stream when video recording is disabled but assessment is started
  const initializeAudioOnly = useCallback(async () => {
    if (isAudioInitialized || microphoneStreamRef.current) {
      logger.info('Audio stream already initialized');
      return;
    }

    try {
      logger.info('Initializing audio-only stream for speech recognition');
      ...
      logger.info('Audio-only stream initialized successfully');
    } catch (error) {
      logger.error('Failed to initialize audio-only stream:', error);
      if (error instanceof Error) {
        if (error.name === 'NotAllowedError') {
          toast.error(
            'Microphone access denied. Please enable permissions for speech recognition.'
          );
        } else if (error.name === 'NotFoundError') {
          toast.error(
            'No microphone found. Speech recognition will not be available.'
          );
        }
      }
    }
  ...
  useEffect(() => {
    ...
    const initMediaStreams = async () => {
      if (!isStarted) {
        logger.info('Assessment not started, skipping media initialization');
        return;
      }
      ...
      try {
        if (videoRecordingEnabled) {
          // Initialize full media streams (video + audio) for video recording
          await initializeMediaStreams();
        } else {
          // Initialize audio-only stream for speech recognition
  ...
  }, [isStarted, videoRecordingEnabled, initializeAudioOnly]);
  ...
  useEffect(() => {
    const fetchAssessment = async () => {
      ...
      try {
        ...
        if (
          assessment?.status !== PublicPracticeAssessmentStatusEnum.IN_PROGRESS
        ) {
          // Assessment is not in progress, redirect to check page
          logger.info('Assessment not in progress, redirecting to check page');
          router.replace(
        ...
        setVideoRecordingEnabled(
          assessment.publicPracticeAssessmentSettings?.videoRecordingEnabled ||
            false
        );
        const proctoring =
          assessment.publicPracticeAssessmentSettings?.proctoringEnabled ||
          false;
        ...
        // Don't load question here - it will be loaded when user clicks "Start"
        // This matches the job AI assessment flow
        setAssessment(assessment);

        // Initialize chunk index from server to handle page refresh correctly
        try {
          const chunks = await publicPracticeAssessmentService.getVideoChunks(
            assessmentId,
            {
          ...
          if (chunks && chunks.length > 0) {
            ...
            const maxChunkIndex = Math.max(...chunks.map((c) => c.chunkIndex));
  ...
  useEffect(() => {
    ...
    const tabSwitchLimit =
      assessment?.publicPracticeAssessmentSettings?.tabSwitchLimit;
      ...
      if (document.hidden && !isHandlingViolation) {
        ...
        try {
          ...
          if (proctoringEnabled) {
            ...
            await publicPracticeAssessmentService.recordProctoringEvent(
              assessment?.id || '',
              'TAB_SWITCHED'
            );
    ...
    const handleWindowBlur = async () => {
      ...
      const WINDOW_BLUR_LIMIT =
        assessment?.publicPracticeAssessmentSettings?.tabSwitchLimit || 3;
  ...
  useEffect(() => {
    ...
    const hasStartedBefore = sessionStorage.getItem(
      `teamcast-practice-assessment-${assessmentId}-started`
    );
  ...
  // Note: This function is defined here but used in useEffect above
  // It's stable due to useCallback with empty deps, so it's safe to use
  ...
  const uploadChunk = async (chunk: Blob) => {
    ...
    try {
      // Get presigned URL from backend with the chunk index, questionId, and sectionId
      const response =
        await publicPracticeAssessmentService.getPresignedUrlForVideoChunk(
          assessmentId,
          chunkIndex,
          questionId,
          sectionId
        );
      const { presignedUrl, gcsUri: finalGcsUri } = response;
      ...
      try {
        ...
        const recordResponse =
          await publicPracticeAssessmentService.recordVideoChunkUpload(
            assessmentId,
            chunkIndex,
            finalGcsUri,
            questionId,
        ...
        logger.warn(
          'Recording chunk upload failed. Continuing without analytics.',
          error
        );
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (
        ...
        (mediaRecorderRef.current as any).chunkInterval
      ) {
        clearInterval((mediaRecorderRef.current as any).chunkInterval);
      }
    };
  }, []);
  ...
  const speakText = async (
    ...
    try {
      ...
      const dialectCode =
        assessmentData?.publicPracticeAssessmentSettings?.interviewDialect ||
        'en-US';
      const voiceGender =
        (assessmentData?.publicPracticeAssessmentSettings
          ?.interviewVoiceGender as 'female' | 'male') || 'female';
  ...
  const handleSubmitAnswer = async (answer: string) => {
    try {
      ...
      const response = await publicPracticeAssessmentService.submitAnswer(
        assessmentId,
        currentQuestion.id,
        answer
      );
      ...
      if (response.shouldEndAssessment || response.isCompleted) {
        logger.info('Assessment complete - preparing to finish', {
          shouldEndAssessment: response.shouldEndAssessment,
          isCompleted: response.isCompleted,
          hasNextQuestion: !!response.nextQuestion,
        });

        // Set processing false before handling completion
        setIsProcessing(false);

        // Wait for state update to complete
        await new Promise((resolve) => setTimeout(resolve, 500));

        // Speak completion message if available
      ...
      // Update the question state AND the refs for next question
  ...
  const getQuestionOnLoad = (): any | undefined => {
    if (!assessment) {
      return;
    }
    ...
    if (
      !assessmentData?.progressState?.currentSectionId ||
      !assessmentData?.progressState?.currentQuestionId
    ) {
      throw new Error('Current section or question ID is required');
    }

    const currentSection = assessmentData.sections?.find(
      (section: any) =>
        section.id === assessmentData.progressState?.currentSectionId
    );

    const question = currentSection?.questions?.find(
      (question: any) =>
        question.id === assessmentData.progressState?.currentQuestionId
    );
  ...
  const handleStart = async () => {
    ...
    // Get the first question from progressState or from the first section
    let firstQuestion: any | undefined;
    try {
      firstQuestion = getQuestionOnLoad();
    } catch (error) {
      logger.warn(
        'Could not get question from progressState, trying first section',
        {
          error: error instanceof Error ? error.message : 'Unknown error',
        }
      );
      // If progressState is not set, get the first question from the first section
      if (assessment.sections && assessment.sections.length > 0) {
        const firstSection = assessment.sections[0];
        if (firstSection.questions && firstSection.questions.length > 0) {
          firstQuestion = firstSection.questions[0];
        }
      }
    }
    ...
    if (firstQuestion) {
      ...
      toast.error('No question found. Please try refreshing the page.');
    }
  };
  ...
  const handleInterviewComplete = async () => {
    ...
    try {
      ...
      if (assessment?.id) {
        await publicPracticeAssessmentService.submitAssessment(assessment.id);
        logger.info('Assessment submitted successfully');

        // Trigger video analysis if video recording was enabled
        if (videoRecordingEnabled) {
          try {
            await publicPracticeAssessmentService.triggerVideoAnalysis(
              assessment.id
            );
            logger.info('Video analysis triggered successfully');
          } catch (error) {
            logger.warn('Failed to trigger video analysis:', error);
            // Don't fail the submission if video analysis fails
          }
        }
      }
      ...
      await speakText(
        'Thank you for completing the practice assessment! Your responses have been submitted for review. Once the results are available, you will be notified and can view them.',
        true
      );

      // Note: Activity logging and analytics may not be needed for practice assessments
      // or can be added later if required
    } catch (error) {
  ...
  const sendHeartbeat = async (assessmentId: string, timeRemaining: number) => {
    ...
    try {
      ...
      await publicPracticeAssessmentService.heartbeat(
        assessmentId,
        duration,
        'IN_PROGRESS'
      );
                ...
                >
                  ...
                  <div className="mb-8 text-center">
                    ...
                    <p className="text-muted-foreground mt-2">
                      Sign in to unlock your results and personalized
                      recommendations
                    </p>
                  </div>
                  ...
                  <div className="bg-card rounded-2xl border p-8 shadow-lg">
                    <div className="flex flex-col gap-8 md:flex-row md:items-start md:justify-between">
                      {/* Left Side - Unlock Results */}
                      <div className="flex-1">
                        <div className="mb-4 flex items-center gap-3">
                          <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#6e55cf]/10">
                            <Lock className="h-5 w-5 text-[#6e55cf]" />
                          </div>
                        ...
                        <p className="text-muted-foreground mb-6 text-sm">
                          {isLoggedIn
                            ? 'View your assessment insights, job match score, and personalised recommendations.'
                            : 'Sign in to view your assessment insights, job match score, and personalised recommendations.'}
                        </p>

                        <Button
                          className="w-full rounded-lg bg-[#6e55cf] py-3 text-white hover:bg-[#5d47b5]"
                          onClick={() => {
                            // If user is already logged in, redirect to practice assessments results page
                            if (isLoggedIn) {
                              router.push(
                                '/app/candidate/practice-assessments'
                              );
                              return;
                            }

                            // Check metadata for redirect logic (for non-logged-in users)
                            const metadataKey = `practice-assessment-${assessmentId}-metadata`;
                            const metadataStr =
                              sessionStorage.getItem(metadataKey);
                            ...
                            if (metadataStr) {
                              try {
                                ...
                                requiresPasswordSetup =
                                  metadata.requiresPasswordSetup ?? true;
                              } catch (e) {
                            ...
                            if (requiresPasswordSetup) {
                              // Redirect to set-password page with practice assessment context
                              const params = new URLSearchParams();
                              params.set('practice_assessment', 'true');
                              ...
                              router.push(
                                `/app/auth/set-password?${params.toString()}`
                              );
                            } else {
                              ...
                              toast.info(
                                'You already have an account. Please log in to view your assessment results.'
                              );
                              router.push(
                                `/app/auth/login?${params.toString()}`
                              );
                            }
                          }}
                        >
                      ...
                      <div className="bg-muted/50 flex-1 rounded-xl border p-5">
                        {/* Assessment Score */}
                        <div className="mb-5">
                          <h3 className="text-foreground mb-3 text-sm font-semibold">
                            Assessment Score
                          </h3>
                          <div className="flex h-24 items-center justify-center rounded-lg bg-[#6e55cf]/5">
                            <span className="text-4xl font-bold text-[#6e55cf]/30">
                              %
                            </span>
                          </div>
                        </div>
                          ...
                          <div className="space-y-2">
                            {[1, 2, 3].map((i) => (
                              <div key={i} className="flex items-center gap-2">
                                <div className="h-2 w-2 rounded-full bg-[#6e55cf]/20" />
                                <div
                                  className="bg-muted-foreground/20 h-3 rounded"
                                  style={{ width: `${70 - i * 15}%` }}
                                />
                              </div>
                            ))}
                            ...
                            <div className="flex items-center space-x-2">
                              <div className="bg-primary text-surface flex h-8 w-8 items-center justify-center rounded-full text-sm font-medium">
                                {(assessment?.sections?.findIndex(
                                  (s: any) =>
                                    s.id === currentQuestion?.sectionId
                                ) ?? -1) + 1}
                              </div>
                              <div>
                            ...
                            <div className="text-text-tertiary hidden items-center space-x-3 text-sm md:flex">
                              {(() => {
                                ...
                                const currentSectionIndex =
                                  assessmentData.sections.findIndex(
                                    (s: any) =>
                                      s.id === currentQuestion?.sectionId
                                  );
                                const isLastSection =
                                  currentSectionIndex ===
                                  assessmentData.sections.length - 1;
                ...
                <div className="mx-auto min-h-148 w-full max-w-4xl px-3 sm:px-4 md:px-6">
                  ...
                  <div className="mt-3 pb-24 sm:mt-4 sm:pb-20 md:mt-8 md:pb-12">
                    {isProcessing ? (
                      ...
                      <div>
                        {/* Speech Recognition Status - Show when listening */}
                        {(isListening || sttListening) && (
                          <motion.div
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
              ...
              {videoRecordingEnabled && (
                <div className="fixed right-4 bottom-4 h-40 w-52 overflow-hidden rounded-2xl bg-white shadow-sm sm:h-48 sm:w-64 md:bottom-6 md:left-6 md:h-64 md:w-96">
                  ...
                  <FaceDetectionOverlay
                    videoRef={videoRef}
                    isEnabled={proctoringEnabled && isStarted}
                    assessmentId={assessmentId || ''}
                  />
                </div>
              )}
