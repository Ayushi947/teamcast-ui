...
interface PublicPracticeAssessmentCheckProps {
  startAssessment: () => void;
  title: string;
  description: string;
  importantInformation: importantInfo[];
  assessmentId: string; // Required for public practice assessments
}

const PublicPracticeAssessmentCheck = ({
  startAssessment,
  title,
  description,
  importantInformation,
  assessmentId,
}: PublicPracticeAssessmentCheckProps) => {
  ...
  // Fetch current terms acceptance status from public practice assessment
  const fetchTermsAcceptanceStatus = useCallback(async () => {
    ...
    try {
      ...
      setTermsAccepted(assessment.termsAccepted || false);
      logger.info(
        'Fetched terms acceptance status from public practice assessment',
        {
          assessmentId: assessment.id,
          status: assessment.termsAccepted,
        }
      );
    } catch (_error) {
      logger.info(
        'Failed to fetch public practice assessment, setting terms accepted to false by default'
      );
      setTermsAccepted(false);
    }
  }, [assessmentId]);
  ...
  // Handle terms acceptance with API call (no authentication required)
  const handleTermsAcceptance = async (accepted: boolean) => {
    ...
    try {
      const result = await publicPracticeAssessmentService.updateTermsAccepted(
        assessmentId,
        accepted
      );
...
export default PublicPracticeAssessmentCheck;
