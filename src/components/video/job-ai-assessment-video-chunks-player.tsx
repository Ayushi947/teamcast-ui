...
interface JobAiVideoChunksService {
  getJobAiAssessmentVideoChunks: (
    assessmentId: string,
    options?: {
      questionId?: string;
      sectionId?: string;
...
interface JobAiAssessmentVideoChunksPlayerProps {
  assessmentId: string;
  service: JobAiVideoChunksService;
  className?: string;
  questionId?: string;
  sectionId?: string;
  includeAnalysis?: boolean;
...
export const JobAiAssessmentVideoChunksPlayer = ({
  assessmentId,
  service,
  className,
  questionId,
  ...
  videoUrl,
  highlightsVideoUrl,
}: JobAiAssessmentVideoChunksPlayerProps) => {
  ...
  useEffect(() => {
    ...
    const loadChunks = async () => {
      ...
      try {
        logger.info(
          'Fetching video chunks for JobAiAssessmentVideoChunksPlayer',
          {
            assessmentId,
          }
        );

        const result = await service.getJobAiAssessmentVideoChunks(
          assessmentId,
          {
            questionId,
            sectionId,
  ...
  > => {
    logger.info(
      'Refreshing video chunks for JobAiAssessmentVideoChunksPlayer',
      {
        assessmentId,
      }
    );
    try {
      const result = await service.getJobAiAssessmentVideoChunks(assessmentId, {
        questionId,
        sectionId,
        includeAnalysis,
        includePlaybackUrls: true,
  ...
  const handleUrlExpired = useCallback(async () => {
    logger.info(
      'Refreshing video chunk URLs for JobAiAssessmentVideoChunksPlayer',
      {
        assessmentId,
      }
    );
